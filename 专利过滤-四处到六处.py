...
必须是csv格式的，且只有一个sheet。输入文件名为了方便固定为《测试验证部专利总览.csv》。输出文件名称为《测试验证部测试一处和二处个人专利完成情况统计.csv》
"""
import pandas

list_username = []
filename_input = '测试验证部专利总览.csv'.decode('gbk')
filename_output = '测试验证部测试四处到六处个人专利完成情况统计.csv'.decode('gbk')
