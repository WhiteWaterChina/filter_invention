...
listDivisionName=['测试一处'.decode('gbk'), '测试二处'.decode('gbk'), '测试三处'.decode('gbk'), '测试四处'.decode('gbk'), '测试五处'.decode('gbk'), '测试六处'.decode('gbk'), '测试七处'.decode('gbk')]
...
class InventionFilterAll(wx.Frame):
    def __init__(self, parent):
        wx.Frame.__init__(self, parent, id=wx.ID_ANY, title=u"专利结果过滤工具", pos=wx.DefaultPosition, size=wx.Size(387, 279),
...
#处理人员名称
        ...
        for item_1 in range(1, total_rows_one):
            ...
            type_invention = sheet_filter_one.cell(item_1, 4).value.replace(u' ', u'').split(",")[0]
...
#write output excel
        ...
        for item_chu in listDivisionName:
            ...
            for index, item_sheet_one in enumerate(namelist["%s" % item_chu]):
                ...
                sheet_now.write(line_count, 1, data_display["%s" % item_chu]['%s' % item_sheet_one]['发明提交数量'.decode('gbk')], formatone)
                sheet_now.write(line_count, 2, data_display["%s" % item_chu]['%s' % item_sheet_one]['发明受理数量'.decode('gbk')], formatone)
                sheet_now.write(line_count, 3, data_display["%s" % item_chu]['%s' % item_sheet_one]['实用新型提交数量'.decode('gbk')], formatone)
                sheet_now.write(line_count, 4, data_display["%s" % item_chu]['%s' % item_sheet_one]['实用新型受理数量'.decode('gbk')], formatone)
